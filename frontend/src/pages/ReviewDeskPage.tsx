...
export function ReviewDeskPage({ onNavigate }: { onNavigate: (href: string) => void }) {
  ...
  const totalLabel = activeFilterCount ? `${rangeStart}-${rangeEnd}` : total.toLocaleString('id-ID');
    ...
    <main className="review-desk-page" style={styles.shell}>
      <section className="card" style={styles.hero}>
        ...
        <div style={styles.totalCard} aria-label="Total package reviews">
          <span style={styles.bigNumber}>{totalLabel}</span>
          <span style={styles.smallLabel}>{activeFilterCount ? `${total.toLocaleString('id-ID')} matched entries` : 'worklist entries'}</span>
        </div>
      </section>
...
const styles: Record<string, CSSProperties> = {
  ...
  pageButton: { minHeight: 38, border: '1px solid rgba(215,209,176,.24)', borderRadius: 999, padding: '0 .9rem', background: 'rgba(215,209,176,.1)', color: 'var(--lp-cream)', fontWeight: 820 },
